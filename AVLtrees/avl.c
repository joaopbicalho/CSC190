...
bool check_avl_tree(avl_node_t *root){
    
    int height_dif;
    
    height_dif = abs(HEIGHT(root->right) - HEIGHT(root->left));
    ...
    if (height_dif > 1)
        ...
        bool left = check_avl_tree(root->left);
        bool right = check_avl_tree(root->right);
        return (left && right);
    }
}    

bool is_avl_tree(bag_t *bag){
    return check_avl_tree(bag->root);
}   


void insert_norot(avl_node_t *root, bag_elem_t elem){

    if (elem < root->elem && root->left == NULL){
        avl_node_t *node = avl_node_create(elem);
        root->left = node;
    }
    else if(elem > root->elem && root->right == NULL){
        avl_node_t *node = avl_node_create(elem);
        root->right = node;
    }
    else{
        if(elem < root->elem){
            insert_norot(root->left, elem);
        }
        else{
            insert_norot(root->right, elem);
        }
    }
    avl_update_height(root);
}
...
bool bag_insert_norot(bag_t *bag, bag_elem_t elem){
    if(bag->root == NULL){
        ...
        insert_norot(bag->root, elem);
    }
}

bool avl_remove_scratch(avl_node_t **root, bag_elem_t elem,
                int (*cmp)(bag_elem_t, bag_elem_t))
{
    bool removed;
